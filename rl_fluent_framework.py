...
class CompressorEnv(gym.Env):
    ...
    def _normalized_to_physical(self, action: np.ndarray) -> tuple[float, float]:
        a_norm = float(np.clip(action[0], 0.0, 1.0))
        amplitude = float(self.a_min + a_norm * (self.a_max - self.a_min))
        if self.rl_config.action_dim == 1:
            return amplitude, self.f_min
        f_norm = float(np.clip(action[1], 0.0, 1.0))
        frequency = float(self.f_min + f_norm * (self.f_max - self.f_min))
        return amplitude, frequency

    def _make_velocity_expression(self, amplitude: float, frequency: float) -> str:
        if self.rl_config.expr_mode == "sin":
            return (
    ...
    def _read_latest_tploss_scaled(self) -> float:
        ...
        tploss_value = float(pd.read_csv(latest_file, sep=r"\s+", skiprows=2).iloc[-1, 2]) * 10.0
    ...
    def reset(self):
        ...
        amplitude, frequency = self._normalized_to_physical(action_init)
    ...
    def step(self, action):
        ...
        amplitude, frequency = self._normalized_to_physical(action)
