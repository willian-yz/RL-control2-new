...
def build_case5_configs() -> tuple[EnvConfig, RLConfig]:
    env_config = EnvConfig(
        ...
        workdir=r"E:\A keti\Case\50theta_b_t1.6_bow20_M0.5_i+8.5_2D_change inlet large_RL_2_simple_action",
        max_decisions=10000, #100
        slice_len = 1, # dt=5e-6s, 每个2e-4s决策一次, 250是因为冲角变化f=20hz
        initial_tploss=0.05,
        processor_count = 4,
    ...
    rl_config = RLConfig(
        ...
        max_delta_amplitude = 1.0, #修改后
        use_delta_action=True,      #修改后
        expr_mode="constant",
        n_steps=2000, #80 #2步训练一次
        batch_size=400,
        n_epochs=5, #参数更新5次
    )
...
def main() -> None:
    ...
    model_path = manager.train(train_steps=10000*20)
