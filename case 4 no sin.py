"""Case 4: RL + Fluent synthetic jet control (refactored for research maintainability)."""
...
class FluentEnvConfig:
    """Configuration container for the CFD-coupled RL environment."""

    cas_path: str
    data_path: str = "./double_sided 70 angle steady_jet0_dt=0.0115s.dat.h5"
    workdir: str = r"D:\LYZ\A case4 with hole jet ag70"
    show_gui: bool = True
    processor_count: int = 56
    slice_len: int = 10
    max_decisions: int = 100

    amplitude_range: tuple[float, float] = (0.0, 100.0)
...
class CompressorEnv(gym.Env):
    """Fluent-coupled compressor control environment (Gym 0.21 style API)."""
    ...
    def __init__(self, config: FluentEnvConfig):
        ...
        self.f_min, self.f_max = self.config.frequency_range
    ...
    def _reset_internal_state(self) -> None:
        ...
        self._frequency_hist: list[float] = []
        self._tploss_hist: list[float] = []
        self._velocity_expr_hist: list[str] = []

    def _normalized_to_physical(self, a_norm: float, f_norm: float) -> tuple[float, float]:
        amplitude = float(self.a_min + np.clip(a_norm, 0.0, 1.0) * (self.a_max - self.a_min))
        frequency = float(self.f_min + np.clip(f_norm, 0.0, 1.0) * (self.f_max - self.f_min))
        return amplitude, frequency

    def _make_velocity_expression(self, amplitude: float, frequency: float) -> str:
        amp_text = f"{amplitude:.2f}"
        freq_text = f"{frequency:.2f}"
        return (
            f"{amp_text}[m s^-1]*sin(2*{self.config.pi}*{freq_text} [s^-1]"
            f"*(t - {self.config.t0}[s]))"
        )
    ...
    def _build_observation(self, a_norm: float, f_norm: float, tploss_scaled: float) -> np.ndarray:
        return np.array(
            [a_norm, f_norm, tploss_scaled, self.t_idx / self.config.max_decisions],
            dtype=np.float32,
        )

    def _compute_reward(self, tploss_scaled: float) -> float:
        baseline_scaled = self.config.baseline_tploss * 10.0
        return baseline_scaled - tploss_scaled
    ...
    def reset(self):
        ...
        a_norm_init, f_norm_init = 0.0, 0.0
        self.a_current, self.f_current = self._normalized_to_physical(a_norm_init, f_norm_init)
        self.current_velocity_expr = self._make_velocity_expression(self.a_current, self.f_current)
        self._set_inlet_velocity(self.current_velocity_expr)

        tploss_scaled = self.config.baseline_tploss * 10.0
        obs = self._build_observation(a_norm_init, f_norm_init, tploss_scaled)

        self._amplitude_hist.append(self.a_current)
        self._frequency_hist.append(self.f_current)
    ...
    def step(self, action):
        ...
        amplitude, frequency = self._normalized_to_physical(a_norm, f_norm)

        velocity_expression = self._make_velocity_expression(amplitude, frequency)
        ...
        self.a_current, self.f_current = amplitude, frequency
        self.last_tploss = tploss_scaled
        self.last_action = np.array([a_norm, f_norm], dtype=np.float32)
        self.t_idx += 1

        obs = self._build_observation(a_norm, f_norm, tploss_scaled)

        self._amplitude_hist.append(amplitude)
        self._frequency_hist.append(frequency)
        self._tploss_hist.append(tploss_scaled)
...
def main() -> None:
    config = FluentEnvConfig(cas_path="double_sided 70 angle steady_jet0_dt=0.0115s.cas.h5")
